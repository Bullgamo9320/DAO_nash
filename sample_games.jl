...
function run_dao_game(n::Int)
    ...
    for i in 1:n
        actions = [0.0, 1.0, 2.0]
        f = (a, others) -> GameResolver.payoff_func(i, a, others, w_values[i])
        push!(players, GameResolver.Player(i, actions, f))
        GameResolver.player_wjs[i] = w_values[i]
    end

    game = GameResolver.Game(players)
    println("Getting Nash Equilibrium...")
    equilibria = GameResolver.get_Nash_equilibrium(game)

    # for (i, eq) in enumerate(equilibria)
    #     println("Equilibrium $i: ", eq)
    # end
...
function DAO_sand(n)
    ...
    latex_table = create_latex_table_old(w_values, nash)
    println(latex_table)
end

function create_latex_table_old(w_values, nash_equilibria)
...
function DAO_game2(n)
    ...
    for i in 1:n
        ...
        # Player インスタンスの作成
        player = GameResolver.Player(i, actions, (action, w_others...) -> f(action, w_values[i], w_others))
        push!(player_list, player)
    end

    # ゲームの作成
    g = GameResolver.Game(player_list)
    ...
    # ナッシュ均衡の計算
    println("Getting Nash Equilibrium:")
    nash = GameResolver.get_Nash_equilibrium(g)
    @info "Nash Equilibrium:", nash
...
function DAO_game3(n)
    ...
    function f(action, w_j, w_others)
        ...
        elseif action == 2  # buy
            new_wj = w_j + buy_unit
            new_total = sum_w + new_wj
            # 将来の持ち分 = new_wj、なので new_wj^2 / new_total^2
            future_utility = buy_unit * (P_l + 0.1 * P) + a * P_l * buy_unit + c * (new_wj^2) / new_total^2 - c * (w_j / (sum_w + w_j))^2
            buy_cost = buy_unit * P
            return future_utility - buy_cost

        else
            return -999999.0  # 不正なアクション
        end
    end
    ...
    player_list = []
    for i in 1:n
        actions = Float64[0, 1, 2]  # leave, stay, buy
        player = GameResolver.Player(i, actions, (action, w_others...) -> f(action, w_values[i], w_others))
        push!(player_list, player)
    end
...
function create_latex_table(w_values, nash_equilibria)
    ...
    \\\\begin{table}[h]
    \\\\centering
    \\\\caption{Nash Equilibrium Simulation Results (3-action version)}
    \\\\begin{tabular}{|c|$(repeat("c|", length(nash_equilibria)))}
    \\\\hline
    $header_row \\\\\\\\
    \\\\hline
    ...
    for i in 1:length(sorted_w_values)
        ...
        strategies_row = [
            ...
            strategy[i] == 1 ? "stay" :
            "buy"
            for strategy in sorted_nash_equilibria
        ]
        latex_table *= "$w_value & " * join(strategies_row, " & ") * " \\\\\\\\n\\\\hline\n"
    end
...
function DAO_game4(n)

    # 定数定義
    P = 0.5                      # 現在のトークン価格
    P_l = P * 0.7                # 売却価格（=価格下落後）
    k = 7                        # ガス代
    a = 0.3                      # 通貨的効用係数
    c = 10000                    # ガバナンス効用係数

    # トークン保有量をベータ分布から生成
    alpha, beta = 0.1, 12000
    w_values = 2.1e9 * rand(Beta(alpha, beta), n)

    # 利得関数の定義
    function payoff(action, w_j, w_others)
        sum_w = sum(w_others)
        total_w = sum_w + w_j
        buy_unit = 0.5 * w_j
        ...
        elseif action == 1  # stay
            governance_util = c * (w_j / total_w)^2
            monetary_util = w_j * (P_l + 0.1 * P) + a * P_l * w_j
            return monetary_util + governance_util
    ...
    player_list = []
    for j in 1:n
        actions = [0.0, 1.0, 2.0]  # leave, stay, buy
        f_j = (action, w_others...) -> payoff(action, w_values[j], w_others)
        push!(player_list, GameResolver.Player(j, actions, f_j))
    end

    # ゲームの定義とナッシュ均衡の取得
    g = GameResolver.Game(player_list)

    println("Token holdings for each player:")
    for j in 1:n
        println("Player $j: w = $(w_values[j])")
    end
    ...
    @info "Nash Equilibrium:" nash
    println(create_latex_table(w_values, nash))
end
...
#     players = DAOGameFast.Player[]
#     for i in 1:n
#         actions = [0.0, 1.0, 2.0]  # leave, stay, buy
#         f = (a, others) -> DAOGameFast.payoff(a, w_values[i], others)
#         push!(players, DAOGameFast.Player(i, actions, f))
#     end

#     game = DAOGameFast.Game(players)
#     println("Getting Nash Equilibrium...")
#     equilibria = DAOGameFast.get_nash_equilibria(game)
