...
function DAO_game3(n)
    ...
    function f(action, w_j, w_others)
        ...
        elseif action == 1  # stay
            # 分配項は total_w を用いて統一
            return w_j * (P_l + 0.1 * P) + a * P_l * w_j + c * (w_j / total_w)^2

        elseif action == 2  # buy
            new_wj = w_j + buy_unit
            new_total = sum_w + new_wj
            # 将来の持ち分 = new_wj、なので new_wj^2 / new_total^2
            future_utility = buy_unit * (P_l + 0.1 * P) + a * P_l * buy_unit + c * (new_wj^2) / new_total^2 - c * (w_j / total_w)^2
            buy_cost = buy_unit * P
